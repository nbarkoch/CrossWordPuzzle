...
// Below this many playable (4-6 letter) words, the fast refill is treated as a
// near dead-end and escalated to the full spread-aware refill so the player is
// never handed a starved grid.
const WORD_WAVE_MIN_PLAYABLE_AFTER_REFILL = 4;
// When the findable words span fewer than this many distinct 3x3 zones they are
// clustered in one part of the board; the fast refill rerolls to spread them.
const WORD_WAVE_MIN_SPREAD_AFTER_REFILL = 3;
// Upper bound on how many times a single word-clear rerolls the cheap fast
// refill looking for a healthier board. Kept small so even the worst beat stays
// frame-affordable — we never fall back to the heavy deep refill inline.
const WORD_WAVE_FAST_REFILL_MAX_TRIES = 8;
...
// Number of distinct 3x3 zones the given moves physically span (1..9). A low
// count means every findable word is crammed into one corner of the board.
const getPlayableSpread = (moves: WordWaveMove[]) =>
  new Set(moves.flatMap(move => move.path.map(getLocationBucketKey))).size;

// Summarizes a refilled board straight from the moves the fast refill already
// computed (no extra findWordWaveMoves pass): how many playable words there are
// and how far they spread. Cheap enough to run on every reroll candidate.
const summarizeRefillPlayability = (moves: WordWaveMove[]) => {
  const playable = getUniqueMovesByWord(moves).filter(
    move =>
      move.word.length >= WORD_WAVE_PLAYABLE_MIN_WORD_LENGTH &&
      move.word.length <= WORD_WAVE_PLAYABLE_MAX_WORD_LENGTH,
  );

  return {count: playable.length, spread: getPlayableSpread(playable)};
};

const getRefillHealthScore = (summary: {count: number; spread: number}) =>
  summary.count * 100 + summary.spread * 6;
...
export const applyWordWaveTimedSelectionFast = (
  ...
  let best = refillWordWaveBoardFast(board, path);
  let bestSummary = summarizeRefillPlayability(best.moves);
  let refillAttempts = best.attempts;

  // The fast refill is a single injection + random fill with no candidate
  // selection, so it can occasionally leave the player almost nothing to find,
  // or with every findable word clustered in one corner. Rather than escalate to
  // the heavy deep refill (dozens of fully rescored candidates — enough to
  // stutter the game when run inline), reroll the CHEAP fast refill a bounded
  // number of times and keep the richest, best-spread result. A healthy first
  // roll skips the loop entirely, so normal play keeps its original cost; only
  // the rare thin/clustered beat pays for a few extra rerolls, all frame-cheap.
  for (
    let tries = 1;
    tries < WORD_WAVE_FAST_REFILL_MAX_TRIES &&
    (bestSummary.count < WORD_WAVE_MIN_PLAYABLE_AFTER_REFILL ||
      bestSummary.spread < WORD_WAVE_MIN_SPREAD_AFTER_REFILL);
    tries += 1
  ) {
    const candidate = refillWordWaveBoardFast(board, path);
    const candidateSummary = summarizeRefillPlayability(candidate.moves);

    refillAttempts += candidate.attempts;

    if (getRefillHealthScore(candidateSummary) > getRefillHealthScore(bestSummary)) {
      best = candidate;
...
// Engine internals shared with the dev-only wordWaveAnalysis.ts. Not part of the
// public runtime API — do not import these from app code.
